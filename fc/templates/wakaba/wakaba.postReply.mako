        ...
        <tr>
            ...
            <td class="reply" id="reply${post.id}">
                ...
                <label>
                    ...
                    <span class="reflink">
                    %if c.board:
                        <a href="/${thread.id}#i${post.id}" ${c.currentUserCanPost and """onclick="doQuickReplyForm(event,%s,%s)" """ % (thread.id,post.id) or ""}>#${g.OPT.secondaryIndex and post.secondaryIndex or post.id}</a>
                    %else:
                        <a href="javascript:insert('&gt;&gt;${post.id}')" ${c.currentUserCanPost and """onclick="doQuickReplyForm(event,%s,%s)" """ % (thread.id,post.id) or ""}>#${g.OPT.secondaryIndex and post.secondaryIndex or post.id}</a>
                    %endif
