...
class FcaController(OrphieBaseController):
    ...
    def editUserByPost(self, pid):
        ...
        addLogEntry(LOG_EVENT_USER_GETUID, "Viewed UID for user '%s' from post '<a href='/%s#i%s'>%s</a>'. Reason: %s" % (post.uidNumber, post.parentid and post.parentid or post.id, pid, pid, reason))
